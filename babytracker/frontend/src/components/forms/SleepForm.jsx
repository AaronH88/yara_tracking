...
export default function SleepForm({ event, onSaved, onCancel }) {
  const { selectedBaby } = useBaby();
  const { persona } = usePersona();

  const [users, setUsers] = useState([]);
  const [userId, setUserId] = useState(event?.user_id ?? persona?.id ?? "");
  const [sleepType, setSleepType] = useState(event?.type ?? "nap");
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const url = event
        ? `/api/v1/babies/${selectedBaby.id}/sleeps/${event.id}`
